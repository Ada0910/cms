...
<script>
    ...
    function add() {
        ...
        layer.open({
            ...
            shade: 0.8,
            area: [w + 'px', h + 'px'],
            content: '/backstage/down/classify/add' //iframe的url
        });
    }
    ...
    function edit(id) {
        ...
        layer.open({
            ...
            shade: 0.8,
            area: [w + 'px', h + 'px'],
            content: '/backstage/down/classify/edit?id=' + id //iframe的url
        });
    }
    ...
    function del(ids) {
        ...
        $.post('/admin/down/classify/delete', {ids: ids}, function (result) {
            if (result.success) {
                layer.closeAll();
                layer.msg('删除成功');
                reload_data();
    ...
    function table_edit_update(id, field, value) {
        $.post('/admin/down/classify/update?' + field + '=' + value, {id: id}, function (result) {
            if (result.success) {
                layer.msg('修改成功');
            } else {
                layer.closeAll();
    ...
    function update_isUse(id, isUse) {
        $.post(' /admin/down/classify/update', {id: id, isUse: isUse}, function (result) {
            if (result.success) {
                layer.msg('修改成功');
            } else {
                layer.msg('修改失败');
...
<div class="layui-form" style=" ">
    ...
    <div class="layui-table-toolbar" style="margin-bottom: 3px;">
        <div class="layui-btn-group">
            ...
            <button class="layui-btn layui-btn-sm" onclick="delSelected()"><i class="layui-icon">&#xe640;</i>删除选中项
            </button>
    ...
    <script type="text/html" id="format_isUse">
        <input type="checkbox" name="content" value="{{d.id}}" lay-skin="switch" lay-text="是|否" lay-filter="tp_isUse" {{
               d.isUse== 1 ? 'checked' : '' }}>
    </script>
