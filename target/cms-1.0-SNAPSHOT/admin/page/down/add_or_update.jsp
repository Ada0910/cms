...
<script>
$(function(){
	if("${down.downClassifyId}"!=""){
		$("#downClassifyId").val("${down.downClassifyId}");
	}
	
	if("${down.displayMode}"!=""){
		$("#displayMode").val("${down.displayMode}");
	}
	...
	function save() {
		...
		var downClassifyId = $("#downClassifyId").val();
		...
		$.post(save_url, {
			...
			contentNoTag : app.contentNoTag,downClassifyId:downClassifyId,
			summary : app.summary,
			isUse : isUse,
	...
	function uploadFile(file){
	    $.ajaxFileUpload({
	        url : '/admin/down/addThumbnailImage', //用于文件上传的服务器端请求地址
	        ...
	        success : function(result) //服务器成功响应处理函数  
	        {
	            ...
	            if(result.success){
	            	...
	            	$("#thumbnailImage_div").empty();
	            	$("#thumbnailImage_div").append('<img style="width: 200px;" src="'+result.path+'" />');
	            }
	        },  
		...
		<div>
			<div class="layui-form layui-form-pane">
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<select name="downClassifyId" id="downClassifyId"
							lay-filter="downClassifyId">
							<c:forEach var="downClassify" items="${downClassifyList}">
								<option value="${downClassify.id}">${downClassify.name}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="layui-form-item"
					...
					<div class="layui-input-block">
						...
						<div id="thumbnailImage_div" style="min-height: 10px;">
							<img style="width: 200px;" alt=""
								src="${down.thumbnailImage }" />
						</div>
					</div>
				</div>
